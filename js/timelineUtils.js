				...
				if (nFeatures > 0) {
					...
					array.forEach(response.features, lang.hitch(this, function (feature) {
						var ext = feature.geometry.getExtent(),
								...
								mapFullName = feature.attributes[this.config.ATTRIBUTE_MAP_FULL_NAME],
								author = feature.attributes[this.config.ATTRIBUTE_AUTHOR], //my variable
						...
						var tooltipContent = "<img class='tooltipThumbnail' src='" + this.setThumbnailUrl(this.token, objID) + "'>" +
								...
								"<div class='tooltipHeader'>" + mapFullName + " </div>" +
								//REMOVED formattedDate from tooltip
								//"<div class='tooltipHeader'>" + mapName + " (" + formattedDate + ")</div>" +
								// I ADD the Year back to tooltip
								//"<div class='tooltipHeader'>" + mapName + " (" + year + ")</div>" +
			...
			if (_sel.length) {
				if (_sel[0].row !== undefined) {
					var row = _sel[0].row,
							...
							//my added field
							author = _timelineData[row].author,
					...
					qt.execute(q, lang.hitch(this, function (rs) {
						var extent = rs.features[0].geometry.getExtent(),
								mapName = rs.features[0].attributes[this.config.ATTRIBUTE_MAP_FULL_NAME],
								dateCurrent = rs.features[0].attributes[this.config.ATTRIBUTE_DATE],
